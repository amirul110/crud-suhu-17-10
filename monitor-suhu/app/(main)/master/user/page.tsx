...
const UserPage = () => {
    ...
    const handleEditUser = async (e: React.FormEvent) => {
        ...
        try {
            console.log('✏️ Updating user:', selectedUser.id, 'with data:', formData);
            ...
            if (json.data?.status === '00') {
                ...
                toastRef.current?.showToast('99', json.data?.message || 'Gagal update user');
            }
        } catch (err) {
            console.error('❌ Edit user error:', err);
            const errorMessage = err instanceof Error ? err.message : 'Gagal update user';
            toastRef.current?.showToast('99', errorMessage);
        }
    };
